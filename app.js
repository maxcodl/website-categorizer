...
const keywords = require('./categoriesKeywords.json');
const openaiPrompts = require('./prompt.json');
...
function categorizeWebsite(text) {
    ...
    for (const category in keywords) {
        ...
        const matchedKeywords = categoryKeywords.filter(keyword => text.includes(keyword));
    ...
    if (Object.keys(categoryCounts).length > 0) {
        const bestCategory = Object.keys(categoryCounts).reduce((a, b) => categoryCounts[a] > categoryCounts[b] ? a : b);
        return bestCategory;
    }
...
async function main(url) {
    ...
    if (category) {
        ...
        const matchedKeywords = keywords[category].filter(keyword => extractedText.includes(keyword));
...
const enteredURL = 'https://bbc.com'; // Replace with the URL you want to categorize
main(enteredURL);
